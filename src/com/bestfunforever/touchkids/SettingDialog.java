...
public class SettingDialog extends BaseDialog {
	...
	public SettingDialog(SimpleBaseGameActivity context, Camera camera, float ratio) {
	...
	public void onLoadResource() {
		...
		this.mSoundBitmapTextureAtlas = new BitmapTextureAtlas(context.getTextureManager(), 527, 388,
				TextureOptions.BILINEAR);
		this.mSoundTextureRegion = BitmapTextureAtlasTextureRegionFactory.createTiledFromAsset(
				this.mSoundBitmapTextureAtlas, context, "sound.png", 0, 0, 2, 1); // 64x32
		this.mSoundBitmapTextureAtlas.load();

		this.mMusicBitmapTextureAtlas = new BitmapTextureAtlas(context.getTextureManager(), 527, 388,
				TextureOptions.BILINEAR);
		this.mMusicTextureRegion = BitmapTextureAtlasTextureRegionFactory.createTiledFromAsset(
				this.mMusicBitmapTextureAtlas, context, "music.png", 0, 0, 2, 1); // 64x32
	...
	protected float initContent(Rectangle mContentRect) {
		Rectangle rect1 = new Rectangle(0, ScoreDialog.margin * ratio, 0, 0, context.getVertexBufferObjectManager());
		rect1.setColor(Color.TRANSPARENT);
		Text soundText = new Text(0, 0, mFont, context.getString(R.string.sound) + " ",
				context.getVertexBufferObjectManager());
		soundSateText = new Text(300 * ratio, 0, mFont, context.getString(R.string.state_off) + " ",
				context.getVertexBufferObjectManager());
		CheckBox soundCheckbox = new CheckBox(soundSateText.getX() + soundSateText.getWidth(), 0, ratio, true,
		...
		soundCheckbox.setOnCheckedChangeListenner(new ICheckedChange() {
			...
			public void onCheckedChange(boolean checked) {
				// TODO Auto-generated method stub
				soundSateText.setText(checked ? context.getString(R.string.state_on) : context
						.getString(R.string.state_off));
			}
		});
		float rect1Width = soundCheckbox.getX() + soundCheckbox.getWidth();
		float rect1Height = Math.max(soundText.getHeight(), soundCheckbox.getHeight());
		...
		Rectangle rect2 = new Rectangle(0, rect1.getY() + rect1.getHeight() + ScoreDialog.margin * ratio, 0, 0,
				context.getVertexBufferObjectManager());
		rect2.setColor(Color.TRANSPARENT);
		Text musicText = new Text(0, 0, mFont, context.getString(R.string.music) + " ",
				context.getVertexBufferObjectManager());
		musicSateText = new Text(300 * ratio, 0, mFont, context.getString(R.string.state_off) + " ",
				context.getVertexBufferObjectManager());
		CheckBox musicCheckbox = new CheckBox(musicSateText.getX() + musicSateText.getWidth(), 0, ratio, true,
				mSoundTextureRegion, context.getVertexBufferObjectManager());
		musicCheckbox.setOnCheckedChangeListenner(new ICheckedChange() {

			@Override
			public void onCheckedChange(boolean checked) {
				musicSateText.setText(checked ? context.getString(R.string.state_on) : context
						.getString(R.string.state_off));
			}
		});
		float rect2Width = musicCheckbox.getX() + musicCheckbox.getWidth();
		float rect2Height = Math.max(musicText.getHeight(), musicCheckbox.getHeight());
